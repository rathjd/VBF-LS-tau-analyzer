...
while (true) do {
	...
	unsigned int gensize = genparticlehelper.size();
	fsr_weight = 1;
	for (unsigned int i = 0; i<gensize; ++i) {
		const genparticlehelperplus_s *lepton = genparticlehelper[i];
		...
		const genparticlehelperplus_s * boson = lepton->mother();
		...
		if (lepton->numberOfDaughters==0) { continue; }
			...
			const genparticlehelperplus_s * otherlepton = genparticlehelperplus[otherleptonKey];
