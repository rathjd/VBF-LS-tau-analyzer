...
double myJERetaWeightsUpArray[]  = {1.105, 1.127, 1.150, 1.254, 1.316, 1.458, 1.247};
double myJERetaWeightsDownArray[] = {1.053, 1.071, 1.092, 1.162, 1.192, 1.332, 0.865};

std::vector<double> myJERetaEdges_ (myJERetaEdgesArray, myJERetaEdgesArray + sizeof(myJERetaEdgesArray) / sizeof(double) );
std::vector<double> myJERetaWeights_ (myJERetaWeightsArray, myJERetaWeightsArray + sizeof(myJERetaWeightsArray) / sizeof(double) );
std::vector<double> myJERetaWeightsUp_ (myJERetaWeightsUpArray, myJERetaWeightsUpArray + sizeof(myJERetaWeightsUpArray) / sizeof(double) );
...
for(unsigned int i = 0;i<jet.size();++i){
	...
	double eta = jet[i].eta;
	double jetweight = 0.;
	bool isMatchedtoGenJet = false;
	TLorentzVector recojet_v;
	TLorentzVector genjet_v;
	recojet_v.SetPtEtaPhiE(jet[i].pt, jet[i].eta, jet[i].phi, jet[i].energy);

	for(unsigned int j = 0; j < genparticlehelper.size(); ++j) {
		int id = genparticlehelper[j].pdgId;
		if (!(abs(id) < 9)) continue;
		int status = genparticlehelper[j].status;
		if (status!=3) continue;
		genjet_v.SetPtEtaPhiM(genparticlehelper[j].pt, genparticlehelper[j].eta, genparticlehelper[j].phi, genparticlehelper[j].mass);
		double genRecoDeltaR = genjet_v.DeltaR(recojet_v);

		if (genRecoDeltaR < 0.3) {
			isMatchedtoGenJet = true;
			break;	
		}
	}

	if (isMatchedtoGenJet && eta > myJERetaEdges_[0] && eta < myJERetaEdges_[nbins]) {
		for (unsigned int j=1; j<=nbins; ++j) {
			if (eta >  myJERetaEdges_[j]) continue;
			if (!JERuncUp && !JERuncDown) jetweight = myJERetaWeights_[j-1];
			if (JERuncUp && !JERuncDown) jetweight = myJERetaWeightsUp_[j-1];
			if (!JERuncUp && JERuncDown) jetweight = myJERetaWeightsDown_[j-1];
			//cout << "eta: "<< eta << " c: " << jetweight << " Oldpt: " << jet[i].pt << endl;
			if (jetweight > 0.) {
				double newRecoPt = max( 0. , genjet_v.Pt() + jetweight * ( recojet_v.Pt() - genjet_v.Pt() ));
			       	//cout << "newpt: "<< newRecoPt << endl;	
				jet[i].pt = newRecoPt;
				recojet_v.SetPtEtaPhiE(jet[i].pt, jet[i].eta, jet[i].phi, jet[i].energy);
				jet[i].energy = recojet_v.Energy();	
			} 
			break;
		}
	}
