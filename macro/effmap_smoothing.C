...
TH2F *histL=((TH2F*)(_file0->Get("h2_taufakerate_loose_eff")))->Clone("h2_taufakerate_loose_eff_smoothed");

for(unsigned int x=0; x<histL->GetNbinsX(); x++)
  {
  	unsigned int count=0;
	double value = -1;
	double error = -1;
	int end=-1;
  	for(unsigned int y=0; y<histL->GetNbinsY(); y++)\
	  {
	  	double binC=histL->GetBinContent(x+1,y+1);
		if(y==end)break;
		if((binC>0 && y>0) || y>0) count++;
		if(count==1)
		  {
		    double lowEdge=histL->GetYaxis()->GetBinLowEdge(y+1);
		    double highEdge=histL->GetYaxis()->GetBinLowEdge(histL->GetNbinsY()-y+1);
		    TF1 *const=new TF1("const","[0]");
		    TH1F *projection=(TH1F*)histL->ProjectionY("py",x+1,x+1);
		    projection->Fit(const,"RF","",lowEdge,highEdge);
		    value = const->GetParameter(0);
		    error = const->GetParError(0);
		    std::cout<<lowEdge<<" to "<<highEdge<<" yielding "<<value<<" +/- "<<error<<std::endl;
		    end=histL->GetNbinsY()-y;
		  }
		if(value>0)
		  {
		    histL->SetBinContent(x+1,y+1,value);
		    histL->SetBinError(x+1,y+1,error);
		  }
	  }
  }
histL->Write();
