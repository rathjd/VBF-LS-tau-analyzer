...
int main(int argc, char** argv)
{
  ...
  //int nevents = 100;
  unsigned int nevents = stream.size();
...
cout << "Total number of selected data: " << selectedEvents_ << " [events], corresponding to acceptance: [" << originalAcceptance*100 << " +- " << 100*sqrt( originalAcceptance*(1.-originalAcceptance)/originalEvents_) << "] %" << endl;
...
if (npairs>0) {
	cout << "\tNumber of eigenvectors for uncertainty estimation: " << npairs << endl;
	double wplus = 0.;
	double wminus = 0.;
	unsigned int nplus = 0;
	unsigned int nminus = 0;
	for (unsigned int j=0; j<npairs; ++j) {
		double wa = weightedSelectedEvents_[2*j+1]/events_central-1.;
		double wb = weightedSelectedEvents_[2*j+2]/events_central-1.;
		if (nnpdfFlag) {
			if (wa>0.) {
				wplus += wa*wa;
				nplus++;
	...
	cout << "\tRelative uncertainty with respect to central member: +" << std::setprecision(4) << 100.*wplus << " / -" << std::setprecision(4) << 100.*wminus << " [%]" << endl;
} else {
...
100*sqrt((acc2_central/waverage-acc_central*acc_central)/originalEvents_)
	...
	cout << "\ti.e. [" << std::setprecision(4) << 100*xi/originalAcceptance << " +- " << std::setprecision(4) << 100*deltaxi/originalAcceptance << "] % relative variation with respect to the original PDF" << endl;
...
cout << ">>>> End of PDF weight systematics summary >>>>" << endl;
