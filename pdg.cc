...
string pdg::particleName(int pdgid)
{
  if ( first )
    {
      first = false;
               ...
               int           depth)
{
  ...
  stream << std::setw(10) << pdg::particleName(ppid);
  ...
  // Loop over daughters
  for (int i=firstDaughter[index]; i <= lastDaughter[index]; i++)
    pdg::printTree(stream, 
                   i,
                   nhep,
                   Pdgid,
                   Status,
...
double
pdg::deltaPhi(double phi1, double phi2)
{
...
double
pdg::deltaR(double eta1, double phi1, double eta2, double phi2)
{
