...
int main(int argc, char** argv)
{
  ...
  */
  ...
  h2_Trigger->GetXaxis()->SetTitle("real #tau 1 iso, 0Tight, 1Medium, 2Loose, 3NoIso");
  h2_Trigger->GetYaxis()->SetTitle("real #tau 2 iso, 0Tight, 1Medium, 2Loose, 3NoIso");
  h2_Trigger->Sumw2(); 
  
  TH2F* h2_NoTrigger = new TH2F("h2_NoTrigger", "NOT triggered in isolations", 5, -1.5, 3.5, 5, -1.5, 3.5);
  h2_NoTrigger->GetXaxis()->SetTitle("real #tau 1 iso, 0Tight, 1Medium, 2Loose, 3NoIso");
  h2_NoTrigger->GetYaxis()->SetTitle("real #tau 2 iso, 0Tight, 1Medium, 2Loose, 3NoIso");
  h2_NoTrigger->Sumw2();
  ...
  for(int entry=0; entry < nevents; ++entry)
	...
	std::vector<int> nones;
	  ...
	  for(unsigned int t =0;t<tau.size();++t){
	    ...
	    for(int i=0; i<genTau.size(); i++){
	      double dRtemp=deltaR(tau[t].eta, tau[t].phi, genTau[i]->eta, genTau[i]->phi);
	...
	int realTau1=-1;
	int realTau2=-1;
	if(tights.size()==2){realTau1=0; realTau2=0;}
	else if(mediums.size()==2){realTau1=1; realTau2=1;}
	else if(looses.size()==2){realTau1=2; realTau2=2;}
	else if(nones.size()==2){realTau1=3; realTau2=3;}
	else if(tights.size()==1){realTau1=0;
	  if(mediums.size()==1) realTau2=1;
	  if(looses.size()==1) realTau2=2;
	  if(nones.size()==1) realTau2=3;
	}  
	else if(mediums.size()==1){realTau1=1;
	  if(looses.size()==1) realTau2=2;
	  if(nones.size()==1) realTau2=3;
	}  
	else if(looses.size()==1){ realTau1=2; if(nones.size()==1) realTau2=3;}
	//if(realTau1>0)std::cout<<realTau1<<", "<<realTau2<<" - "<<baselineObjectSelectionCollection.passedTrigger<<std::endl;  
	if(baselineObjectSelectionCollection.passedTrigger) h2_Trigger->Fill(realTau1, realTau2, weight);
	else h2_NoTrigger->Fill(realTau1, realTau2, weight);
    }
	  //Event Count
	  ofile.count("NoCuts");
	  // ------------------------
